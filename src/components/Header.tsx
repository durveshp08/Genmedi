...
import { canAccessTab, ROLE_TAB_ACCESS } from "../types/auth";
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const consumerTabs: { id: TabType; label: string; icon: React.ComponentType<{ className?: string }>; badge?: string }[] = [
    ...
    { id: "health_vault", label: "Health Vault", icon: Heart },
  ];
  ...
  const visibleConsumerTabs = consumerTabs.filter((tab) =>
    canAccessTab(userRole, tab.id)
  );

  const visibleOpsTabs = operationsTabs.filter((tab) =>
    canAccessTab(userRole, tab.id)
  );

  const isOpsActive = operationsTabs.some((t) => t.id === activeTab);
  const showOpsPortal = visibleOpsTabs.length > 0;
            ...
            {isAuthenticated && user ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 px-2.5 py-1.5 rounded-xl border border-[#e5eeff] hover:border-[#006a61] transition-colors cursor-pointer bg-white"
                >
                  <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#006a61] to-[#86f2e4] flex items-center justify-center text-white text-xs font-bold shadow-sm">
                    {user.name.charAt(0).toUpperCase()}
                  </div>
                  <div className="hidden md:block text-left">
                    <div className="text-xs font-semibold text-[#0b1c30] leading-tight truncate max-w-[100px]">
                      {user.name}
                    </div>
                    <span className={`text-[9px] font-bold uppercase px-1 py-0.5 rounded ${ROLE_BADGE_COLORS[user.role] || "bg-gray-100 text-gray-700"}`}>
                      {user.role}
                    </span>
                  </div>
                  <ChevronDown className="w-3 h-3 text-[#76777d] hidden md:block" />
                </button>

                {showUserMenu && (
                  <>
                    <div className="fixed inset-0 z-40" onClick={() => setShowUserMenu(false)} />
                    <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-[#e5eeff] py-2 z-50 animate-in fade-in slide-in-from-top-2">
                      <div className="px-3 py-2 border-b border-[#eff4ff]">
                        <div className="text-sm font-semibold text-[#0b1c30]">{user.name}</div>
                        <div className="text-[11px] text-[#76777d]">{user.email}</div>
                        <span className={`inline-block mt-1 text-[9px] font-bold uppercase px-1.5 py-0.5 rounded ${ROLE_BADGE_COLORS[user.role] || "bg-gray-100 text-gray-700"}`}>
                          {user.role}
                        </span>
                      </div>
                      <div className="py-1">
                        <button
                          type="button"
                          onClick={() => {
                            onTabChange("health_vault");
                            setShowUserMenu(false);
                          }}
                          className="w-full text-left px-3 py-2 text-xs text-[#0b1c30] hover:bg-[#eff4ff] flex items-center gap-2 cursor-pointer"
                        >
                          <UserCircle className="w-4 h-4 text-[#76777d]" />
                          My Health Vault
                        </button>
                        <button
                          type="button"
              ...
              <button
                type="button"
                onClick={onOpenAuth}
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold bg-gradient-to-r from-[#006a61] to-[#008578] text-white shadow-sm hover:shadow-md transition-all cursor-pointer"
              >
      ...
      <div className="sm:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t border-[#dce9ff] px-2 py-1.5 flex items-center justify-around shadow-lg">
        ...
        {isAuthenticated ? (
          ...
          <button
            type="button"
            onClick={onOpenAuth}
            className="flex flex-col items-center gap-0.5 py-1 px-2 rounded-lg text-[10px] font-medium text-[#006a61]"
          >
